import { getHashedColor, IsNullOrWhitespace, IsUsableNumber, lerp } from "@/shared/utils";
...
const VisBarSegment = ({ item, isCurrentTrack, audioAPI, mediaPlayer, styleOverride }: VisBarSegmentProps) => {
    ...
    const handleClickColoredBar = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (isCurrentTrack) {
            ...
            void recordAction({ feature: ActivityFeature.media_player_bar_seek_vis_segment, songId: item.songContext?.publicId, fileId: item.file?.publicId });
    ...
    const handleClickContainer = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (!isCurrentTrack) {
            ...
            void recordAction({ feature: ActivityFeature.media_player_bar_play_vis_segment, songId: item.songContext?.publicId, fileId: item.file?.publicId });
    ...
    const tooltipContent = `${title.displayIndex}${title.title}${title.subtitle ? ` - ${title.subtitle}` : ""}`;

    return IsNullOrWhitespace(tooltipContent) ? (
        mainContent
    ) : (
