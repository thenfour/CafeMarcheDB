import { DateTimeRange, getLocalCalendarWindow, getDateTimeRangeCalendarProjection, gMillisecondsPerHour } from "@/shared/time";
...
export const useEventsForDateRange = (dateRange: DateTimeRange | null, timeZone: string) => {
    ...
    const filterSpec = React.useMemo(() => {
        ...
        const startDate = dateRange.getStartDateTime(now);
        const lastDate = dateRange.getLastDateTime(now);
        const endDateExclusive = new Date(lastDate.getFullYear(), lastDate.getMonth(), lastDate.getDate() + 1);
        return {
            refreshSerial: 1,
            quickFilter: "",
            calendarWindow: getLocalCalendarWindow(startDate, endDateExclusive, timeZone),
    ...
    }, [dateRange, now, dashboardContext.eventStatus, timeZone]);
            ...
            try {
                const transformed = enrichedEvents.flatMap(event => event.segments
                    ...
                    .map(segment => ({
                        ...
                        dateRange: timeZone ? getDateTimeRangeCalendarProjection(getEventSegmentDateTimeRange(segment), timeZone) : getEventSegmentDateTimeRange(segment),
                    })));
    ...
    }, [enrichedEvents, filterSpec, dashboardContext.eventStatus, timeZone]);
