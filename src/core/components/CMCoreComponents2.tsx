...
export const EventTextLink = (props: {
    event: Prisma.EventGetPayload<{ select: { id: true, name: true, startsAt: true } }>,
    className?: string | undefined,
}) => {
    return <a rel="noreferrer" target="_blank" className={props.className} href={getURIForEvent(props.event)}>
        {db3.EventAPI.getLabel(props.event)}
        {/* {props.event.startsAt?.toLocaleDateString()} {props.event.name} */}
    </a >;
};