...
import { CalculateSongMetadata } from "src/core/components/song/SongComponentsBase";
...
import type { SongSearchClient } from "../../db3/shared/entities/song/songViews";

type SongListItemProps = {
    index: number;
    song: SongSearchClient;
...
export const SongListItem = (props: SongListItemProps) => {
    ...
    const taggedFiles = (props.song.taggedFiles || []).flatMap(association => (
        association.file ? [{ ...association, file: association.file }] : []
    ));
    const hasFileTagSignificance = (association: typeof taggedFiles[number], significance: string) => (
        (association.file.tags || []).some(tag => tag.fileTag.significance === significance)
    );
    const partitions = taggedFiles.filter(file => hasFileTagSignificance(file, db3.FileTagSignificance.Partition));
    const recordings = taggedFiles.filter(file => hasFileTagSignificance(file, db3.FileTagSignificance.Recording));
    const otherFiles = taggedFiles.filter(file => (
        !hasFileTagSignificance(file, db3.FileTagSignificance.Partition)
        && !hasFileTagSignificance(file, db3.FileTagSignificance.Recording)
    ));
            ...
            <div className="searchBody">
                ...
                {hasAnyFooterInfo && (
                    <div className="lengthBpmLine row">
                        ...
                        {hasPartitions && <div className="partitionCount fieldItem">{gIconMap.LibraryMusic()} {partitions.length} {SelectEnglishNoun(partitions.length, "partition", "partitions")}</div>}
                        {hasRecordings && <div className="recordingCount fieldItem">{gIconMap.PlayCircleOutline()} {recordings.length} {SelectEnglishNoun(recordings.length, "recording", "recordings")}</div>}
                        {hasOtherFiles && <div className="otherFilesCount fieldItem">{gIconMap.AttachFile()} {otherFiles.length} {SelectEnglishNoun(otherFiles.length, "unsorted file", "unsorted files")}</div>}
                    </div>
                )}
            </div>
        </AppContextMarker>
