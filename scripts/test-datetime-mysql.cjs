...
async function main() {
    ...
    const databaseName = `cmdb_datetime_test_${randomBytes(8).toString("hex")}`;
    const target = new URL(source);
    target.pathname = `/${databaseName}`;
    if (!/^cmdb_datetime_test_[a-f0-9]{16}$/.test(databaseName) || target.pathname === source.pathname) throw new Error("Invalid disposable database target.");
