...
- Generic reordering is deny-by-default. A table must opt in through schema-owned `sortOrderPolicy` metadata, require an explicit caller-supplied row-ID scope, and declare either global ordering or one exact grouping column. Scope IDs are bounded, unique, and must contain both operation endpoints. The mutation loads only those IDs inside the declared active group, requires every supplied row to be visible to the fresh database actor, and preauthorizes each changed persisted row and its sort-order field before the first write. It applies insertion-style movement only within that scope and reuses the scope's existing numeric slots, so hidden, paginated, tenant-separated, and otherwise out-of-scope rows are neither fetched nor renumbered. The scoped updates and aggregate audit entry execute in one serializable transaction. The five existing UI consumers supply exactly their rendered reorderable IDs; event song lists are grouped by `eventId`, and ungrouped setlist plans explicitly use `groupId: null`. Interactive reordering no longer doubles as whole-table duplicate repair; a future uniqueness/normalization effort must define lane-specific constraints, insertion allocation, nullable-group behavior, and legacy cleanup explicitly.
...
- Verification: 20 focused BA-S003 tests cover table identity and opt-in, fresh grants, explicit-scope isolation and slot preservation, hidden/cross-group scope rejection, per-affected-row preflight, song-list deletion and cascade auditing, self and delegated attendance, independent invitation authority, stale grants, malformed/cross-event segments, and generic-DB3 attendance bypasses. `yarn test:auth` and `yarn test` pass (207 tests); `yarn tsc --noEmit`; focused ESLint; `yarn build`.
- Commit/PR:
...
- [x] Compare the complete desired authorization state, not only array length.
- [x] Include the automatic public-permission union.
- [x] Clear permissions when the role is removed.
- [x] Refresh `isSysAdmin` independently.
- [x] Handle deleted/deactivated users.
...
- [x] Equal-count permission changes take effect.
- [x] Removing a role removes its permissions.
- [x] Removing sysadmin authority removes the bypass.
- [x] Deactivating/deleting a user invalidates active sessions.
- [x] Role-permission revocation affects every user of that role.
