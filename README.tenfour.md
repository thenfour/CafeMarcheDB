...
- `blitz prisma migrate dev --name <migration_name>` very often clobbers database, but not always.
  creates a migration based off your `schema.prisma` file. can also be run
  without specifying name on the CLI (and it will ask you).
  - **IMPORTANT NOTE** when doing this on Windows, Prisma will generate migration sql files
    that have the wrong case; deploying that will be an ERROR on the target server. You have to manually correct this.
